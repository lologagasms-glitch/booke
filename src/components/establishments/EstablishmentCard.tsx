...
const EstablishmentCard = ({ establishment, firstImageUrl }: EstablishmentCardProps) => {
  ...
  return (
    <article className="group relative bg-white rounded-2xl shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 hover:border-blue-200">
      {/* Image on top */}
      <div className="relative w-full h-56 md:h-64">
        ...
        <div className="absolute top-4 left-4">
          <span className="inline-flex items-center gap-1.5 bg-white/80 backdrop-blur-sm text-blue-700 text-xs font-semibold px-3 py-1.5 rounded-full shadow">
        ...
        <div className="absolute top-4 right-4 flex items-center gap-1 bg-white/80 backdrop-blur-sm px-2.5 py-1.5 rounded-full shadow">
          ...
          <span className="text-xs font-semibold text-gray-800">{establishment.etoiles}</span>
        </div>
      </div>
      ...
      <div className="p-5 md:p-6 lg:p-7 flex flex-col justify-between">
        <div>
          ...
          <div className="mb-3">
            ...
            >
              <h3 className="text-xl md:text-2xl font-bold text-gray-900 group-hover/link:text-blue-700 transition-colors duration-200">
                {establishment.nom}
              </h3>
            </Link>
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {establishment.services.slice(0, 5).map((service, idx) => (
              <span
                key={idx}
                className="inline-flex items-center gap-1.5 bg-gray-50 text-gray-700 text-xs px-3 py-1.5 rounded-full border border-gray-200"
              >
        ...
        <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-100">
          ...
          <Link
            href={`/${locale}/etablissements/${establishment.id}`}
            className="inline-flex items-center gap-2 bg-blue-700 hover:bg-blue-800 text-white text-sm font-medium px-4 py-2 rounded-lg transition-all duration-200 shadow hover:shadow-lg"
          >
