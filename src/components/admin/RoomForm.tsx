...
export default function RoomForm({ etablissementId, id }: { etablissementId: string; id?: string }) {
  ...
  // ✅ CORRECTION : Reset les valeurs quand les données sont chargées (services compris)
  useEffect(() => {
    if (defaultValuesChambre) {
      reset(defaultValuesChambre);
    }
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-8">
            ...
            <div className="space-y-6 border-b border-gray-100 pb-8 mb-8">
              ...
              <Controller
                name="services"
                control={control}
                // ❌ defaultValue SUPPRIMÉ - géré par reset()
                render={({ field }) => (
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {serviceOptions.map((service) => {
                      // ✅ CORRECTION : Utiliser field.value qui contient les valeurs du reset()
            ...
            <div className="space-y-6">
              <label className="flex items-center gap-3 p-4 border rounded-md cursor-pointer hover:bg-gray-50">
                <input
                  type="checkbox"
                  {...register('disponible')}
                  // ✅ Utiliser watchedDisponible pour le style si besoin
