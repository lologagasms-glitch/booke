...
import {
  ...
  ChevronLeftIcon,
  ChevronRightIcon,
  PhotoIcon, // Icône pour les médias
...
/* -------------------------------------------------- */
/*  Page Liste Admin - Version Médias                 */
/* -------------------------------------------------- */
export default function EstablishmentMediaTable() {
      ...
      <header className="bg-white/80 backdrop-blur sticky top-0 z-10 border-b border-sand-200">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <HomeModernIcon className="w-8 h-8 text-blue-700" />
            <h1 className="font-display text-2xl text-gray-800">Établissements - Gestion des Médias</h1>
          </div>
          <Link
            href={`/${locale}/admin/etablissements/new`}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-700 text-white hover:bg-blue-800 transition"
          >
...
/* -------------------------------------------------- */
/*  Carte individuelle - Version Médias               */
/* -------------------------------------------------- */
function Card({ etablissement, locale }: { 
  etablissement: Etablissement & { firstImage?: string|null }; 
  locale: string;
}) {
  ...
  return (
    <div className="group relative bg-white rounded-2xl shadow-md hover:shadow-xl transition-shadow overflow-hidden">
      ...
      <div className="p-4">
        ...
        {/* Actions - Boutons Ajouter médias et Chambres côte à côte */}
        <div className="mt-6 flex gap-3 justify-center">
          <Link
            href={`/${locale}/admin/etablissements/${id}/medias/edit`}
          ...
          <Link
            href={`/${locale}/admin/etablissements/${id}/chambres/medias`}
            className="inline-flex items-center gap-2 px-4 py-3 rounded-full
                       ...
                       group/button"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 transition-transform group-hover/button:scale-110" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-4m-5 0H3m2 0h4M9 7h6m-6 4h6m-6 4h6" />
            </svg>
            Chambres
          </Link>
        </div>
