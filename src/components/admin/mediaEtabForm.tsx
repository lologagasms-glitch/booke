...
export default function MediaEtablissementForm({ etablissementId, onSuccess }: MediaEtablissementFormProps) {
  ...
  const TypeBadge = ({ type }: { type: 'image' | 'video' }) => (
    <span className={`absolute top-2 left-2 z-10 px-2 py-1 text-xs font-semibold border rounded ${
      type === 'video' ? 'text-red-600 border-red-600 bg-white' : 'text-blue-600 border-blue-600 bg-white'
    }`}>
    ...
    <div className="border border-gray-200 rounded-lg p-4 space-y-3 bg-white shadow-sm hover:shadow-md transition-shadow">
      <div className="relative">
        ...
        {media.type === 'video' ? (
          ...
          <div 
            className="relative w-full aspect-square bg-gray-50 rounded-lg overflow-hidden cursor-zoom-in hover:bg-gray-100 transition-colors"
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 px-3 py-2 text-red-600 border border-red-200 rounded-lg hover:bg-red-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
      >
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex justify-between items-center pb-4 border-b">
          <h2 className="text-3xl font-bold text-gray-900 tracking-tight">Gestion des médias de l'établissement</h2>
          <button
            type="submit"
            disabled={saveMutation.isPending || newMedias.length === 0}
            className="flex items-center gap-2 px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-gray-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
          >
        ...
        {/* Messages d'erreur */}
        {globalError && (
          <div className="p-4 border border-red-300 text-red-700 rounded-lg flex items-center gap-2" role="alert">
        ...
        {queryError && (
          <div className="p-4 border border-yellow-300 text-yellow-700 rounded-lg flex items-center gap-2" role="alert">
            <ExclamationCircleIcon className="w-5 h-5" />
            <span className="text-sm">Impossible de charger les médias existants</span>
          </div>
        )}
        ...
        <section className="space-y-6 pt-6 border-t">
          <h3 className="text-lg font-semibold text-gray-900">Ajouter de nouveaux médias</h3>
          ...
          <div className="space-y-2">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-gray-900 focus:border-gray-900"
              ...
              <button
                ...
                className="px-5 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-gray-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
              >
          ...
          {newMedias.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {newMedias.map((field, index) => (
                <div key={field.id} className="border border-gray-200 rounded-lg p-4 space-y-3 bg-white shadow-sm">
                  <div className="relative">
                  ...
                  <div className="flex justify-between items-center text-xs text-gray-500">
                    ...
                    <button
                      ...
                      className="ml-3 p-2 text-gray-500 hover:text-red-600 hover:bg-gray-50 rounded-lg transition-colors"
      ...
      {zoomedImage && (
        ...
        >
          ...
          >
            ...
            {/* Contrôles de zoom - Barre horizontale minimaliste */}
            <div className="absolute -top-12 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-white rounded-lg px-3 py-2">
              <button
            ...
            {/* Image */}
            <div className="overflow-auto max-h-screen max-w-screen flex items-center justify-center p-4">
              <img
                src={zoomedImage}
                alt="Image agrandie"
